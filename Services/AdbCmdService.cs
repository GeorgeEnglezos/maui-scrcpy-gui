...
public static class AdbCmdService
{
    ...
    public static async Task<CmdCommandResponse> RunScrcpyCommand(string command)
    {
        ...
        if (string.IsNullOrEmpty(DeviceService.Instance.SelectedDevice.DeviceId))
        {
        ...
        command = $"scrcpy.exe -s {DeviceService.Instance.SelectedDevice.DeviceId} {command} ";

        ProcessStartInfo startInfo = new ProcessStartInfo
        {
            FileName = "cmd.exe",
            Arguments = $"/c \"{command}\"",
            WorkingDirectory = DeviceService.Instance.ScrcpyPath,
    ...
    public static async Task<CmdCommandResponse> RunAdbCommandAsync(CommandEnum commandType, string? command)
    {
        ...
        try
        {
            ...
            {
                var deviceToUseForCommand = DeviceService.Instance.SelectedDevice.DeviceId;
    ...
    public static List<ConnectedDevice> GetAdbDevices()
    {
        ...
        try
        {
            var process = new Process
            {
                ...
                {
                    FileName = Path.Combine(DeviceService.Instance.ScrcpyPath, "adb"),
                    Arguments = "devices -l",
                    RedirectStandardOutput = true,
                    UseShellExecute = false,
                    CreateNoWindow = true
        ...
        if (DeviceService.Instance.ConnectedDevices.Count == 0) DeviceService.Instance.SelectedDevice = new ConnectedDevice();
        DeviceService.Instance.ConnectedDevices = GetCodecsEncodersForEachDevice(list);
        return DeviceService.Instance.ConnectedDevices;
        ...
        foreach (var device in devices)
        {
            ...
            try
            {
                var process = new Process
                {
                    StartInfo = new ProcessStartInfo
                    {
                        FileName = Path.Combine(DeviceService.Instance.ScrcpyPath, "scrcpy"),
                        WorkingDirectory = DeviceService.Instance.ScrcpyPath,
    ...
    {
        if (IsIpAddress(DeviceService.Instance.SelectedDevice.DeviceId))
        {
            return DeviceService.Instance.SelectedDevice.DeviceId;
        }

        foreach (var device in DeviceService.Instance.ConnectedDevices)
        {
    ...
    public static void SetScrcpyPath()
    {
        if (string.IsNullOrEmpty(DataStorage.StaticSavedData.AppSettings.ScrcpyPath))
            DeviceService.Instance.ScrcpyPath = "";
        else
        {
            DeviceService.Instance.ScrcpyPath = DataStorage.StaticSavedData.AppSettings.ScrcpyPath;
            DataStorage.ValidateAndCreatePath(DeviceService.Instance.ScrcpyPath);
        }
    }
}